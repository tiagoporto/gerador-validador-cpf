...
describe('Format function', () => {
  describe('No value', () => {
    it('return valid formatted CPF 137.686.636-63', () => {
      // @ts-expect-error
      expect(format() === undefined).toBeTruthy()
    })
  })
  ...
  describe('Type Number', () => {
    describe('Default Param', () => {
      it('return valid formatted CPF 137.686.636-63', () => {
        expect(format(13768663663) === '137.686.636-63').toBeTruthy()
      })
    })
  ...
  describe('Type String', () => {
    describe('Default Param', () => {
      ...
      it('Should return a valid CPF formatted XXX.XXX.XXX-XX passing formatted CPF', () => {
        expect(format('137.686.636-63') === '137.686.636-63').toBeTruthy()
      })

      it('Passando dígitos', () => {
        format('137686.636639')
        expect(format('137686.636639') === undefined).toBeTruthy()
        expect(spy).toHaveBeenCalledWith(ERROR_MESSAGE.more)
      })

      it('Faltando dígitos', () => {
        expect(format('136.6636-63') === '137.686.636-63').toBeFalsy()
        expect(spy).toHaveBeenCalledWith(ERROR_MESSAGE.fewer)
      })

      it('Com outros caracteres', () => {
        expect(format('13a6.6636-63') === '137.686.636-63').toBeFalsy()
        expect(spy).toHaveBeenCalledWith(ERROR_MESSAGE.fewer)
      })
    })
    ...
    describe('Checker Param', () => {
      it('Should return a valid CPF formatted XXXXXXXXX-XX', () => {
        expect(format('13768663663', 'checker') === '137686636-63').toBeTruthy()
      })
    })
    ...
    describe('Digits Param', () => {
      it('Should return a valid CPF formatted XXXXXXXXXXX', () => {
        expect(
          format('137.686.636-63', 'digits') === '13768663663'
        ).toBeTruthy()
      })
    })
  })
